...
-- Dump della struttura di tabella film.attore
...
) ENGINE=InnoDB AUTO_INCREMENT=26790 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Dump dei dati della tabella film.attore: ~18 rows (circa)
/*!40000 ALTER TABLE `attore` DISABLE KEYS */;
INSERT INTO `attore` (`codattore`, `nome`, `annonascita`, `nazionalita`) VALUES
...
-- Dump dei dati della tabella film.film: ~18 rows (circa)
...
-- Dump della struttura di tabella film.proiezione
CREATE TABLE IF NOT EXISTS `proiezione` (
  ...
  CONSTRAINT `FK_proiezioni_sale` FOREIGN KEY (`codsala`) REFERENCES `sala` (`codsala`) ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Dump dei dati della tabella film.proiezione: ~15 rows (circa)
/*!40000 ALTER TABLE `proiezione` DISABLE KEYS */;
INSERT INTO `proiezione` (`codproiezione`, `codfilm`, `codsala`, `incasso`, `dataproiezione`) VALUES
...
CREATE TABLE IF NOT EXISTS `recita` (
  ...
  CONSTRAINT `FK_recita_attori` FOREIGN KEY (`codattore`) REFERENCES `attore` (`codattore`) ON UPDATE CASCADE,
  CONSTRAINT `FK_recita_film` FOREIGN KEY (`codfilm`) REFERENCES `film` (`codfilm`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Dump dei dati della tabella film.recita: ~24 rows (circa)
...
-- Dump della struttura di tabella film.sala
...
-- Dump dei dati della tabella film.sala: ~7 rows (circa)
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `recita-view` AS select `attore`.`nome` AS `nome`,`attore`.`annonascita` AS `annonascita`,`attore`.`nazionalita` AS `nazionalitaattore`,`film`.`codfilm` AS `codfilm`,`film`.`titolo` AS `titolo`,`film`.`annoproduzione` AS `annoproduzione`,`film`.`nazionalita` AS `nazionalitafilm`,`film`.`regista` AS `regista`,`film`.`genere` AS `genere`,`film`.`durata` AS `durata` from ((`attore` join `recita` on(`attore`.`codattore` = `recita`.`codattore`)) join `film` on(`film`.`codfilm` = `recita`.`codfilm`)) WITH CASCADED CHECK OPTION;
